...
MAINPROJECT="mmgr-geocoder-proyect"
SNAPSHOT="miner"
#MTYPE="n1-highcpu-8"
MTYPE="f1-micro"
DISK_SIZE="10"
...
do {
  ...
  gcloud compute --project $PROJECT ssh --zone $ZONE $INSTANCE --command "nohup gcloud-cryptomine/start.sh gcloud-cryptomine/pools/itns_1cpu_$CONFIG 14 16 & " & > /dev/null 2>&1 &
  fi
} < /dev/null; done
