...
for i in `seq 1 120`
do
	if [ "` top -b -n 1 | head | grep xmr-stak-cpu | awk '{ print $1 }' `" = "" ]
	then 
	nohup cpulimit -l $max gcloud-cryptomine/bin/xmr-stak-cpu $CONFIG_FILE &
	fi
	sleep 10
	top -b -n 1 | head | grep xmr-stak-cpu | awk '{ print $1 }' | while read pid
	do
	cpulimit -l $((RANDOM % ($max-$min+1)+$min)) -p $pid &
	done
done	
top -b -n 1 | head | grep xmr-stak-cpu | awk '{ print $1 }' | xargs kill
sleep 10
done
